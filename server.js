...
app.post('/log', requireAuth, async (req, res) => {
  try {
    ...
    const tok = await getSheetsToken();
    const now = new Date();
    const timeStr = now.toLocaleString('en-GB', { day:'2-digit', month:'2-digit', year:'numeric', hour:'2-digit', minute:'2-digit', second:'2-digit', hour12: false, timeZone:'Asia/Jakarta' });
